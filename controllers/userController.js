const createUserController = ({ connection, primaryAdminEmail }) => {
    ...
    const updateProfile = (req, res) => {
        ...
        if (password && password.length > 0) {
            if (password.length < 6 || !hasNumber.test(password) || !hasSpecial.test(password)) {
                errors.push('If provided, your new password must be at least 6 characters and include a number and a special character.');
            }
        }
